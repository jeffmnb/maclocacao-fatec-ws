...
const messagebird = require("messagebird")(
  "MK69So5f2puffQCyIyHF6ABorsG2RCmypK76"
);
...
router.post("/cadastro", async (req, res) => {
  ...
  if (telefoneExist) {
    return res.json({
      error: true,
      message: "Este telefone já possui uma conta",
    });
  }
...
router.post("/enviasmsvalidacao", async (req, res) => {
  ...
  var params = {
    originator: "Code",
    recipients: [number],
    body: "Seu código de verificação para Maclocação é %token.",
  };

  messagebird.verify.create(params, function (err, response) {
    if (err) {
      console.log(err);
...
router.post("/smsvalidacao", async (req, res) => {
  ...
  if (!userExist) {
    return res.json({
      error: true,
      message: "Este telefone não possui uma conta",
    });
  }
...
router.put("/addfavorites/:idUser", async (req, res) => {
  try {
    ...
    const userRefreshed = await User.findByIdAndUpdate(idUser, {
      imoFavoritos: [...allProps, prop],
    });
  ...
  try {
    ...
    if (propExist) {
      const newFavorites = allFavorites.filter(
        (item) => item._id != body.item._id
      );

      const dataRefreshed = await User.findByIdAndUpdate(idUser, {
        imoFavoritos: newFavorites,
      });
